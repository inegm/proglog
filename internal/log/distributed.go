...
func (l *DistributedLog) setupRaft(dataDir string) error {
	// The finite-state machine that applies the commands given to Raft.
	fsm := &fsm{log: l.log}

	// The log store is where Raft stores those commands.
	...
	// The stable store where Raft stores the cluster's configuration metadata,
	...
	// Raft snapshots are used to recover and restore after a node failure. The
	// snapshots could be stored in object storage (eg. S3). This implementation
	// retains one snapshot.
	retain := 1
	...
	// The transport that Raft uses to connect with the server's peers.
	maxPool := 5
	...
	// LocalID is the only required config, the rest are optional
	config.LocalID = l.config.Raft.LocalID
	// To make our tests faster, we support overriding a handful of timeout
	// configs to speed up Raft (DSwG p 148)
	if l.config.Raft.HeartbeatTimeout != 1 {
...
func (s *StreamLayer) Dial(
	...
	// tell multiplexer this is a raft rpc
